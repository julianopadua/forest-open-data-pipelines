...
# Regex para capturar arquivos anuais: inf_anual_fii_2025.zip
RE_ZIP = re.compile(r"inf_anual_fii_(\d{4})\.zip$", re.IGNORECASE)
...
def load_dataset_cfg(datasets_dir: Path, dataset_id: str) -> DatasetCfg:
    ...
    latest_months = int(raw.get("latest_months", 10))
...
def pick_latest_zip_urls(urls: list[str], limit: int) -> tuple[list[tuple[str, str]], str | None]:
    ...
    for u in urls:
        ...
        # Captura o metadado (zip)
        # Ex: meta_inf_anual_fii.zip
        if "meta_inf_anual_fii" in name.lower():
            meta_url = u
            continue
...
def sync(
    ...
    cfg = load_dataset_cfg(settings.datasets_dir, "cvm/fii_doc_inf_anual")
    ...
    for period, url in zip_urls:
        ...
        local_folder = settings.data_dir / "cvm_fii_doc_inf_anual"
