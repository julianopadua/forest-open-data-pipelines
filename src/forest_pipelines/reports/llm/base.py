...
def _build_prompts(
    ...
    if locale == "pt":
        ...
        user_prompt = (
            ...
            f"{context_json}\n\n"
            "Instruções de saída:\n"
            "- headline: frase curta (máximo 140 caracteres), adequada para título da notícia. "
            "Use somente mês/ano do período mais recente, valor absoluto de focos e UMA comparação principal "
            "(vs ano anterior OU vs média de 5 anos). Não incluir acumulado do ano no headline.\n"
            "- overview: leitura geral da janela de análise recente. "
            "Use somente os números e percentuais já calculados em monthly_analysis.rolling_12_months. "
            "Não calcule percentuais nem deduza valores. Se pct_change ou prior_total forem null, "
            "omita essa comparação em vez de escrever que ela não é calculável. "
            ...
            "Use apenas estatísticas já presentes em monthly_analysis, sem recalcular; "
    ...
    user_prompt = (
        ...
        f"{context_json}\n\n"
            "Output instructions:\n"
            "- headline: short sentence (max 140 characters), suitable for a news title. "
            "Use only the latest month/year, hotspot absolute value and ONE main comparison "
            "(vs previous year OR vs 5-year average). Do not include YTD in the headline.\n"
        "- overview: general reading of the recent analysis window. "
            "Use only the numbers and percentages already calculated in monthly_analysis.rolling_12_months. "
            "Do not calculate percentages or infer values. If pct_change or prior_total are null, "
            "omit that comparison instead of saying it is not calculable. "
        ...
        "Use only statistics already present in monthly_analysis, without recalculating; "
